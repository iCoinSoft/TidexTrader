    ...
    ui(new Ui::ScriptWidget)
{
    ...
    Q_FOREACH(QToolButton *button, menuButtons)button->installEventFilter(this);
...
{
    if(!fileName.isEmpty())saveScriptToFile(fileName);
    delete scriptObject;
    delete ui;
}
...
void ScriptWidget::currencyChanged()
{
    Q_FOREACH(QAction *action,insertEventMenu.actions())
    {
        ...
        if(trString.contains("_BALANCEB_"))
        {
    ...
    {
        QString trString=action->property("TranslationName").toString();
        if(trString.contains("_BALANCEA"))action->setText("trader.get(\"Balance\",\""+baseValues.currentPair.currAStr+"\")");
        else
        if(trString.contains("_BALANCEB"))action->setText("trader.get(\"Balance\",\""+baseValues.currentPair.currBStr+"\")");
    }
}
...
QAction *ScriptWidget::NewFunctionsAction(QString name, QString params)
{
    ...
    if(!params.isEmpty()||name.at(name.size()-1)!=QLatin1Char(')'))params="("+params+")";
    action->setText(name+params);
    action->setToolTip(action->text());
    action->setProperty("TranslationName","SCRIPT_ACTION_"+name.toUpper().replace("\").","_").replace(".","_").replace("(\"","_").replace("\")",""));
...
QAction *ScriptWidget::NewEventsAction(QString name)
{
    ...
    action->setText(name+"()");
    action->setToolTip(action->text());
    action->setProperty("TranslationName","SCRIPT_ACTION_EVENT_"+name.toUpper().replace("\").","_").replace(".","_").replace("(\"","_").replace("\")",""));
    action->setProperty("ScriptName",name);
...
void ScriptWidget::addFunctionClicked()
{
    ...
    if(command==QLatin1String("trader.get(\"BalanceA\")"))command=QString("trader.get(\"Balance\",\"%1\")").arg(baseValues.currentPair.currAStr);
    else
    if(command==QLatin1String("trader.get(\"BalanceB\")"))command=QString("trader.get(\"Balance\",\"%1\")").arg(baseValues.currentPair.currBStr);
    ...
    if(!validPlaceToInsert)
    {
        ...
        if(indexOfQuote<0)indexOfQuote=plainText.length();
        else command.prepend("\n");
...
void ScriptWidget::addEventsClicked()
{
    ...
    if(cursorPos>0)command.prepend("\n");
    ...
    if(cursorPos>0&&cursorPos<plainText.length()-1)
    {
        ...
        {
            if(plainText.at(n)==QLatin1Char('{'))countLeft++;
            else
            if(plainText.at(n)==QLatin1Char('}'))countRight++;
        }
...
bool ScriptWidget::eventFilter(QObject *obj, QEvent *event)
{
    if(event->type()==QEvent::Enter)
    {
        ...
        if(enteredToolButton)
            ...
            {
                if(button==enteredToolButton)continue;
...
{
    if(on==scriptObject->isRunning())return;
    if(!on)scriptObject->stopScript();
    else executeScript(false);
}
...
void ScriptWidget::on_buttonStartStop_clicked()
{
    ...
    if(!executeScript(true)){ui->buttonStartStop->setEnabled(true);return;}
    executeScript(false);
}

void ScriptWidget::setRunningUi(bool on)
{
    if(on)ui->sourceCode->textCursor().clearSelection();
...
{
    bool removed=true;
    if(!fileName.isEmpty()){QFile::remove(fileName);removed=!QFile::exists(fileName);}
    fileName.clear();
    return removed;
}
...
bool ScriptWidget::executeScript(bool testMode)
{
    ...
    if(!testMode && !ui->buttonStartStop->isEnabled())ui->buttonStartStop->setEnabled(true);
    return success;
}

void ScriptWidget::on_validateButton_clicked()
{
    if(executeScript(true))ui->statusLabel->setText("OK");
    saveScriptToFile(fileName);
}
...
{
    if(ui->scriptTabWidget->currentIndex()==0)ui->sourceCode->clear();
    else ui->consoleOutput->clear();
}
...
{
    if(fileSave.isEmpty())fileSave=fileName;
    ui->saveFon->setVisible(false);
    if(QFile::exists(fileSave))QFile::remove(fileSave);
