    ...
    QObject()
{
    ...
    connect(this,SIGNAL(performFileWrite(QString,QByteArray)),&performThread,SLOT(performFileWrite(QString,QByteArray)));
    connect(this,SIGNAL(performFileAppend(QString,QByteArray)),&performThread,SLOT(performFileAppend(QString,QByteArray)));
    connect(this,SIGNAL(performFileReadLine(QString,qint64,quint32)),&performThread,SLOT(performFileReadLine(QString,qint64,quint32)));
    connect(this,SIGNAL(performFileReadLineSimple(QString,quint32)),&performThread,SLOT(performFileReadLineSimple(QString,quint32)));
    connect(this,SIGNAL(performFileRead(QString,qint64,quint32)),&performThread,SLOT(performFileRead(QString,qint64,quint32)));
...
void ScriptObject::fileWrite(QVariant path, QVariant data)
{
    if(testMode)return;
...
void ScriptObject::fileAppend(QVariant path, QVariant data)
{
    if(testMode)return;
...
QVariant ScriptObject::fileReadLine(QVariant path, qint64 seek)
{
    if(testMode)return "";
    fileOpenCount++;
    ...
    QEventLoop waitLoop;
    connect(this, SIGNAL(fileReadExitLoop()),&waitLoop,SLOT(quit()));
    QTimer::singleShot(5000,&waitLoop,&QEventLoop::quit);
    waitLoop.exec();

    QString tempData=arrayFileReadResult[tempFileOperationNumber];
    arrayFileReadResult.remove(tempFileOperationNumber);
    return tempData;
}

QVariant ScriptObject::fileReadLineSimple(QVariant path)
{
    if(testMode)return "";
    fileOpenCount++;
    quint32 tempFileOperationNumber=fileOperationNumber++;
...
QVariant ScriptObject::fileRead(QVariant path, qint64 size)
{
    if(testMode)return "";
    fileOpenCount++;
    quint32 tempFileOperationNumber=fileOperationNumber++;
...
QVariant ScriptObject::fileReadAll(QVariant path)
{
    if(testMode)return "";
    fileOpenCount++;
    quint32 tempFileOperationNumber=fileOperationNumber++;
...
void ScriptObject::fileReadResult(QByteArray data, quint32 tempFileOperationNumber)
{
    fileOpenCount--;
