...
void Exchange_OKCoin::clearVariables()
{
    ...
    if(0.2!=lastFee)emit accFeeChanged(baseValues.currentPair.symbol,0.2);
    lastFee=0.2;
}
...
void Exchange_OKCoin::dataReceivedAuth(QByteArray data, int reqType)
{
	...
	case 109: //trades
        if(data.startsWith("["))
        {
            if(!data.startsWith("[{\"amount\":\""))break;
            ...
            for(int n=0;n<tradeList.count();n++)
            {
                ...
                if(newItem.amount==0.0)continue;
                ...
                if(newItem.isValid())
                {
                    ...
                    {
                        if(newTradesItems->at(i).date!=newItem.date)break;
                    ...
                    if(pos==-1)(*newTradesItems)<<newItem;
                    else
                    {
    ...
    if(!success)
    {
        ...
        if(errorCount>2)
        {
            if(debugLevel)logThread->writeLog("API error: invalid data (ReqType: "+QByteArray::number(reqType)+"):\n"+data,2);
            if(reqType<300)emit showErrorMessage("I:>API error: invalid data (ReqType: "+QByteArray::number(reqType)+")");
        }
    }
    else errorCount=0;
    ...
    if(reqType>=200 && reqType<300)
    {
        ...
        if(!success && data.startsWith("{\"error_code\":"))
        {
            ...
            if(authErrorCount>2)
            {
                ...
                if(debugLevel)logThread->writeLog("API error: "+authError.toLatin1()+" ReqType: "+QByteArray::number(reqType),2);

                if(errorCode==10005 || errorCode==10007 || errorCode==10017)authError.prepend(julyTr("TRUNAUTHORIZED","Invalid API key.")+"<br>");
...
void Exchange_OKCoin::depthSubmitOrder(QString symbol, QMap<double,double> *currentMap ,double priceDouble, double amount, bool isAsk)
{
    if(symbol!=baseValues.currentPair.symbol)return;
...
void Exchange_OKCoin::secondSlot()
{
    ...
    {
    case 0:
        if(!isReplayPending(103))sendToApi(103,"ticker.do?symbol="+baseValues.currentPair.currRequestPair);
        break;
    case 1:
        if(!isReplayPending(202))sendToApi(202,"userinfo.do",true,"api_key="+getApiKey());
        break;
    case 2:
        if(!isReplayPending(109))sendToApi(109,"trades.do?symbol="+baseValues.currentPair.currRequestPair+"&since="+QByteArray::number(lastFetchTid));
        break;
    case 3:
        if(!tickerOnly&&!isReplayPending(204))sendToApi(204,"order_info.do",true,"api_key="+getApiKey()+"&order_id=-1&symbol="+baseValues.currentPair.currRequestPair);
        break;
    case 4:
...
void Exchange_OKCoin::getHistory(bool force)
{
    ...
    if(!isReplayPending(208))sendToApi(208,"order_history.do",true,"api_key="+getApiKey()+"&current_page=1&page_length=200&status=1&symbol="+baseValues.currentPair.currRequestPair);
}
...
void Exchange_OKCoin::buy(QString symbol, double apiBtcToBuy, double apiPriceToBuy)
{
    ...
    QByteArray data="amount="+byteArrayFromDouble(apiBtcToBuy,pairItem.currADecimals,0)+"&api_key="+getApiKey();
    data.append("&price="+byteArrayFromDouble(apiPriceToBuy,pairItem.priceDecimals,0));
...
void Exchange_OKCoin::sell(QString symbol, double apiBtcToSell, double apiPriceToSell)
{
    ...
    QByteArray data="amount="+byteArrayFromDouble(apiBtcToSell,pairItem.currADecimals,0)+"&api_key="+getApiKey();
    data.append("&price="+byteArrayFromDouble(apiPriceToSell,pairItem.priceDecimals,0));
...
void Exchange_OKCoin::sendToApi(int reqType, QByteArray method, bool auth, QByteArray commands)
{
	...
	if(auth)
    {
        commands+="&sign="+QCryptographicHash::hash(commands+"&secret_key="+getApiSign(),QCryptographicHash::Md5).toHex().toUpper();
