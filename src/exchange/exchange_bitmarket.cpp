...
void Exchange_BitMarket::depthSubmitOrder(QString symbol, QMap<double,double> *currentMap ,double priceDouble, double amount, bool isAsk)
{
    if(symbol!=baseValues.currentPair.symbol)return;
...
void Exchange_BitMarket::buy(QString symbol, double apiBtcToBuy, double apiPriceToBuy)
{
    ...
    QByteArray data="trade&market="+symbol.toLatin1()+"&type=buy&amount="+byteArrayFromDouble(apiBtcToBuy,pairItem.currADecimals,0)+"&rate="+byteArrayFromDouble(apiPriceToBuy,pairItem.priceDecimals,0);
...
void Exchange_BitMarket::sendToApi(int reqType, QByteArray method, bool auth, bool sendNow)
{
	...
	if(auth)
    {
		...
		if(sendNow)
            ...
            julyHttp->prepareData(reqType, "POST /api2/", postData, "API-Hash: "+hmacSha512(getApiSign(),postData).toHex()+"\r\n");
	}
	else
	{
